...
//문제는 edge-compatitbe코드를 작성했더니, prisma모듈증에 그걸 싫어하는 모듈이 있어->무시되는 에러인데, 해결안하고 deploy하면 짱 느려짐->해결법:nextauth버전이 업데이트되길기다려라ㅋㅋㅠㅠ->middleware.ts 때문에(edge에서 도는 것이기 때문) 그러니 일단 그것의 이름을 변경해서 에러를 없앤다